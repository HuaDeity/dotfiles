...
function M.info()
  ...
  lines[#lines + 1] = "```"
  ViM.info(lines, { title = "ViM Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command(
    "LazyRoot",
    function() ViM.root.info() end,
    { desc = "ViM roots for the current buffer" }
  )
end
...
function M.get(opts)
  ...
  return ViM.is_win() and ret:gsub("/", "\\") or ret
end
